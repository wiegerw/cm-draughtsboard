...
export class MoveGenerator
{
        ...
        for (let col = 0; col < this.#columns; col++)
        {
          ...
          switch (this.#position[f])
          {
            case PIECE_TYPE.whitePiece:
              ...
              this._doPieceCaptures(row, col)
              this.#position[f] = PIECE_TYPE.whitePiece;
              this._doPieceMove(row, col, -1, -1)
              this._doPieceMove(row, col, -1, 1)
              break

            case PIECE_TYPE.whiteKing:
              this.#position[f] = PIECE_TYPE.empty
              this._doKingCaptures(row, col)
              this.#position[f] = PIECE_TYPE.whiteKing
              this._doKingMoves(row, col, -1, -1)
              this._doKingMoves(row, col, -1, 1)
              this._doKingMoves(row, col, 1, -1)
              this._doKingMoves(row, col, 1, 1)
              break
  ...
  _doKingCaptureDirection(row, col, dr, dc)
  {
  ...
  _doKingCaptures(row, col)
  {
    let foundCaptures = false;
    this.#field.push(this.rc2f(row, col))
    foundCaptures |= this._doKingCaptureDirection(row, col, -1, -1)
    foundCaptures |= this._doKingCaptureDirection(row, col, -1, 1)
    foundCaptures |= this._doKingCaptureDirection(row, col, 1, -1)
    foundCaptures |= this._doKingCaptureDirection(row, col, 1, 1)
    if (this.#frysianMoves)
    {
      foundCaptures |= this._doKingCaptureDirection(row, col, -2, 0)
      foundCaptures |= this._doKingCaptureDirection(row, col, 2, 0)
      foundCaptures |= this._doKingCaptureDirection(row, col, 0, 2)
      foundCaptures |= this._doKingCaptureDirection(row, col, 0, -2)
    }
    if (this.#capturedField.length > 0 && !foundCaptures)
    {
      this._appendCurrentMove()
    }
    this.#field.pop()
  }

  _doKingMoves(row, col, dr, dc)
  {
  ...
  _doPieceCaptureDirection(row, col, dr, dc)
  {
    let foundCaptures = false;
    row += dr
    col += dc
  ...
  _doPieceCaptures(row, col)
  {
    let foundCaptures = false;
    this.#field.push(this.rc2f(row, col))
    if (this.#backwardsCapture || this.#whiteToMove)
    {
      foundCaptures |= this._doPieceCaptureDirection(row, col, -1, -1)
      foundCaptures |= this._doPieceCaptureDirection(row, col, -1, 1)
    }
    if (this.#backwardsCapture || !this.#whiteToMove)
    {
    ...
    if (this.#frysianMoves)
    {
      foundCaptures |= this._doPieceCaptureDirection(row, col, 0, 2)
      foundCaptures |= this._doPieceCaptureDirection(row, col, 2, 0)
      foundCaptures |= this._doPieceCaptureDirection(row, col, 0, -2)
      foundCaptures |= this._doPieceCaptureDirection(row, col, -2, 0)
    }
    if (this.#capturedField.length > 0 && !foundCaptures)
